...
###############################################################################
### diff plot non-commuter mobility by canton                               ###
###############################################################################

# read data
dta <- read_csv(file=here::here("Data","MIP_data","Non_commuter_trips_ColumnG","non_commuter_trips_canton_per_day_2020.csv"))
...
diff_plot <-  ggplot(melt_df, aes( x = Date,
  ...
  # axes title
  ylab("Log of non-commuter trips") + 
...
# save plot
ggsave(filename = "./Plots/parallel_trends/diff_mobility_by_canton_noncommuters.pdf", plot = diff_plot, width = 8, height = 3)