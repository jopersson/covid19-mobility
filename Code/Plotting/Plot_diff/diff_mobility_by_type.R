...
col_names_by_type <- c("total_trips_2020","train_trips_2020","highway_trips_2020","road_trips_2020")
df_sum <- aggregate(dta[col_names_by_type], by=list(Date=dta$date), FUN=sum)
...
# colors
colors <- c("#d34f53","#e5921a","#73c2c0","#884c6e")
...
diff_plot <-  ggplot(df_sum_long, aes( x = Date,
  ...
  annotate("point",
           ...
           color = colors,
           x = start_date,
           y = c(df_sum[1,2],df_sum[1,3],df_sum[1,4],df_sum[1,5])) +
  ...
  # variable text (by type)
  annotate("text",
           fontface = "bold",
           x = start_date,
           y = c(df_sum[1,2]-1500000,df_sum[1,3]-1400000,df_sum[1,4]+2750000),
           label = c("Total","Train","Road"),
           color = colors[1:3],
           hjust = 0,
           size = 2.5) +
  
  # variable text (by type - highway)
  annotate("text",
           fontface = "bold",
           x = start_date,
           y = df_sum[1,5]-4250000,
           label = "Highway",
           color = colors[4],
           hjust = 0,
           size = 2.5) +
...
diff_plot <- diff_plot + lapply(c(18000000,24000000,30000000), function(i) { geom_segment(aes(x=start_date, 
                                                                                              xend=end_date+.5, 
                                                                                              y=i, 
                                                                                              yend=i), 
                                                                                          size=0.1, 
...
# save plot
ggsave(filename = "./Plots/Fig_1/diff_mobility_by_type.pdf", plot = diff_plot, width = 6, height = 3)