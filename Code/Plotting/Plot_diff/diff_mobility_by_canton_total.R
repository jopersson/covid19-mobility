...
###############################################################################
### diff plot total mobility by canton                                      ###
###############################################################################

# read data
dta <- read_csv(file=here::here("Data","MIP_data","Total_trip_counts_ColumnB","total_trip_count_canton_per_day_2020.csv"))
...
diff_plot <-  ggplot(melt_df, aes( x = Date,
  ...
  annotate("point",
           ...
           y = c(16.25, 16.25, 17, 17, 16.6, 16.25, 16.25))


# horizontal grid lines
diff_plot <- diff_plot + lapply(seq(9,16,1), function(i) { geom_segment(aes(x=start_date, 
                                                                            xend=end_date+.5, 
                                                                            y=i, 
                                                                            yend=i), 
                                                                        size=0.1, 
...
# save plot
ggsave(filename = "./Plots/parallel_trends/diff_mobility_by_canton_total.pdf", plot = diff_plot, width = 8, height = 3)